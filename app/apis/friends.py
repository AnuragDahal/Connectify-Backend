...
async def send_friend_request(
    friend_email: Annotated[EmailStr, Form(..., description="Email of the friend to add")],
    user_email: EmailStr = Depends(get_email_from_token)
):
...
async def accept_friend_request(
    friend_email: Annotated[EmailStr, Form(..., description="Email of the friend to accept")],
    user_email: EmailStr = Depends(get_email_from_token)
):
...
async def show_friends(email: EmailStr = Depends(get_email_from_token)):
    response = await FriendsHandler.HandleShowFriends(email)
    return response


@router.get("/requests", status_code=status.HTTP_200_OK)
async def show_friend_requests(user_email: EmailStr = Depends(get_email_from_token)):
